...
create_room.addEventListener('click',async function(){
    try {
        // Automatically use current page's origin for API calls
        ...
        console.log(error)
        // If API call fails, still allow creating room (for local development)
        getInRoom()
    }
})
join_room_btn.addEventListener('click',getInRoom)

// Automatically detect WebSocket URL for online rooms
function getOnlineRoomsWebSocketURL() {
  const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
  const hostname = window.location.hostname;
  const port = window.location.port || (protocol === 'wss:' ? '443' : '8000');
  const wsHost = hostname === 'localhost' ? '127.0.0.1' : hostname;
  return `${protocol}//${wsHost}:${port}/ws/online-rooms/`;
}

const onlinerooms = document.getElementById('onlinerooms')
const home_urls = getOnlineRoomsWebSocketURL();
